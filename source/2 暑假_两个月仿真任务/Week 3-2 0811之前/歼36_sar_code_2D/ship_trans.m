...
stl_file = fullfile(script_dir, 'j36_model.stl');

% 转换后的歼-36模型中：X 轴对应翼展、Z 轴对应机身长度、Y 轴对应高度。
% 下面保留 6 m 的小型实验模型长度，并按原 FBX 轮廓比例设置翼展。
plane_length_m   = 6.0; % 机身长度，最终对应方位向尺寸
plane_wingspan_m = 6.9; % 翼展，最终对应距离向尺寸

% 散射点数量。原 SHIP.m 的计算量较大，建议先用 400~800 点。
num_scatter_points = 3000;
...
% 将 FBX/STL 的 [X, Y, Z] = [翼展, 高度, 长度]，
% 重排为本脚本使用的 [X, Y, Z] = [翼展, 长度, 高度]。
V = V(:, [1, 3, 2]);
...
% 纯二维版本只缩放 X/Y，并在采样前把整个网格压平到 Z=0。
% 因此原模型的高度和垂直侧面不会影响二维散射点密度。
...
fprintf('缩放后二维尺寸：翼展 %.3f m，长度 %.3f m\n', ...
    max(V(:,1))-min(V(:,1)), ...
    max(V(:,2))-min(V(:,2)));
...
[P] = sampleMeshSurface(F, V, num_scatter_points);
...
%% 6. 显示模型俯视图与二维散射点
figure_mesh = figure('Color','w','Name','歼-36三角网格俯视图');
...
xlabel('模型 X / 翼展方向 (m)');
ylabel('模型 Y / 机身方向 (m)');
title(sprintf('歼-36三角网格俯视图：%d 个三角面', size(F,1)));

figure_scatter = figure('Color','w','Name','歼-36二维散射点');
scatter(my_data(:,2), my_data(:,1), 16, [0.25, 0.25, 0.25], 'filled');
axis equal;
grid on;
xlabel('距离向 (m)');
ylabel('方位向 (m)');
title(sprintf('歼-36纯二维散射点分布：%d 点，尺寸约 %.1f m × %.1f m', ...
...
%% ====================== 局部函数 ======================
function P = sampleMeshSurface(F, V, N)
