...
% 方案 B：0 状态占 30%，1 状态占 70% 的非平衡编码。
% 0 状态对应相位 0、反射系数 +1；1 状态对应相位 pi、反射系数 -1。
zero_state_fraction = 0.30;
phase_in_period = mod(phase_command, 2*pi);
modulation_1bit_30_70 = -ones(size(phase_command));
modulation_1bit_30_70(phase_in_period < 2*pi*zero_state_fraction) = 1;

reflection_coefficient_30_70 = ...
    reflection_amplitude * modulation_1bit_30_70;
sr_metasurface_30_70 = reflection_coefficient_30_70 .* sr_plain;
...
metasurface_30_70_raw_energy = sum(abs(sr_metasurface_30_70).^2, 'all');
raw_energy_ratio_30_70 = ...
    metasurface_30_70_raw_energy / (plain_raw_energy + eps);

actual_zero_fraction_balanced = mean(modulation_1bit(:) > 0);
actual_zero_fraction_30_70 = mean(modulation_1bit_30_70(:) > 0);

fprintf('1-bit 调制基频：快时间 %+.3f MHz，慢时间 %+.3f Hz\n', ...
    range_mod_frequency_hz/1e6, azimuth_mod_frequency_hz);
fprintf('平衡编码：0/1 状态比例 %.2f%% / %.2f%%\n', ...
    100*actual_zero_fraction_balanced, 100*(1-actual_zero_fraction_balanced));
fprintf('30%%/70%% 编码：0/1 状态比例 %.2f%% / %.2f%%\n', ...
    100*actual_zero_fraction_30_70, 100*(1-actual_zero_fraction_30_70));
fprintf('平衡编码/裸机原始回波能量比：%.6f（设定 %.6f）\n', ...
    raw_energy_ratio, reflection_power_efficiency);
fprintf('30%%/70%% 编码/裸机原始回波能量比：%.6f（设定 %.6f）\n', ...
    raw_energy_ratio_30_70, reflection_power_efficiency);
...
fprintf('对平衡编码超表面飞机执行 RD 成像...\n');
[img_metasurface, ~, ~] = rdFocus( ...
    sr_metasurface, t, tm, dt, c, R0, Tp, Kr, lambda, v_x, Ka, PRF);

fprintf('对 30%%/70%% 编码超表面飞机执行 RD 成像...\n');
[img_metasurface_30_70, ~, ~] = rdFocus( ...
    sr_metasurface_30_70, t, tm, dt, c, R0, Tp, Kr, lambda, v_x, Ka, PRF);
...
metasurface_30_70_metrics = imageMetrics( ...
    img_metasurface_30_70, Range_axis_ground, Az_axis);

fprintf('\n========== 成像指标 ==========\n');
printMetrics('未加超表面', plain_metrics);
printMetrics('加 1-bit 超表面（平衡编码）', metasurface_metrics);
printMetrics('加 1-bit 超表面（30%/70% 0/1 编码）', ...
    metasurface_30_70_metrics);

metrics_table = table( ...
    {'未加超表面'; '加1-bit超表面（平衡编码）'; ...
     '加1-bit超表面（30%/70%编码）'}, ...
    repmat({char(resolution_mode)}, 3, 1), ...
    [plain_metrics.peak_amplitude; metasurface_metrics.peak_amplitude; ...
     metasurface_30_70_metrics.peak_amplitude], ...
    [plain_metrics.total_energy; metasurface_metrics.total_energy; ...
     metasurface_30_70_metrics.total_energy], ...
    [plain_metrics.peak_range_m; metasurface_metrics.peak_range_m; ...
     metasurface_30_70_metrics.peak_range_m], ...
    [plain_metrics.peak_azimuth_m; metasurface_metrics.peak_azimuth_m; ...
     metasurface_30_70_metrics.peak_azimuth_m], ...
    [plain_metrics.area_3db_m2; metasurface_metrics.area_3db_m2; ...
     metasurface_30_70_metrics.area_3db_m2], ...
    'VariableNames', {'case_name','resolution_mode','peak_amplitude', ...
    'total_energy','peak_range_m','peak_azimuth_m','area_3db_m2'});

%% 9. 显示裸机和两种超表面编码的三幅 SAR 图
comparison_reference = max([max(abs(img_plain(:))), ...
    max(abs(img_metasurface(:))), ...
    max(abs(img_metasurface_30_70(:)))]) + eps;
...
figure_metasurface = createResultFigure( ...
    'Figure 2 - 加 1-bit 超表面（平衡编码）');
plotSarDb(img_metasurface, Range_axis_ground, Az_axis, ...
    display_dynamic_range_db, comparison_reference, display_half_width_m);
title(sprintf(['加 1-bit 超表面（平衡编码）- 假目标偏移 ' ...
    '地距 %+.1f m / 方位 %+.1f m'], ...
    false_range_offset_m, false_azimuth_offset_m));

figure_metasurface_30_70 = createResultFigure( ...
    'Figure 3 - 加 1-bit 超表面（30%/70% 0/1 编码）');
plotSarDb(img_metasurface_30_70, Range_axis_ground, Az_axis, ...
    display_dynamic_range_db, comparison_reference, display_half_width_m);
title(sprintf(['加 1-bit 超表面（0状态30%% / 1状态70%%）- 假目标偏移 ' ...
    '地距 %+.1f m / 方位 %+.1f m'], ...
    false_range_offset_m, false_azimuth_offset_m));
...
metasurface_30_70_png_file = fullfile(run_dir, 'sar_metasurface_30_70.png');
metasurface_30_70_fig_file = fullfile(run_dir, 'sar_metasurface_30_70.fig');
...
if exist('exportgraphics', 'file') == 2
    ...
    exportgraphics(figure_metasurface_30_70, ...
        metasurface_30_70_png_file, 'Resolution', 200);
else
    saveas(figure_plain, plain_png_file);
    saveas(figure_metasurface, metasurface_png_file);
    saveas(figure_metasurface_30_70, metasurface_30_70_png_file);
end
savefig(figure_plain, plain_fig_file);
savefig(figure_metasurface, metasurface_fig_file);
savefig(figure_metasurface_30_70, metasurface_30_70_fig_file);
writetable(metrics_table, csv_file);

save(mat_file, 'img_plain', 'img_metasurface', 'img_metasurface_30_70', ...
    'R_axis_slant_relative', 'Range_axis_ground', 'Az_axis', ...
    'plain_metrics', 'metasurface_metrics', 'metasurface_30_70_metrics', ...
    'resolution_mode', ...
    'false_range_offset_m', 'false_azimuth_offset_m', ...
    'reflection_power_efficiency', 'reflection_amplitude', ...
    'zero_state_fraction', 'actual_zero_fraction_balanced', ...
    'actual_zero_fraction_30_70', ...
    'plain_raw_energy', 'metasurface_raw_energy', ...
    'metasurface_30_70_raw_energy', 'raw_energy_ratio', ...
    'raw_energy_ratio_30_70', ...
