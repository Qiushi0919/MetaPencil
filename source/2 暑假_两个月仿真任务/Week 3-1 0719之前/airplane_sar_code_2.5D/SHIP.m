...
% 近程机载聚束几何：斜距约 1 km，可在普通电脑上仿真小飞机。
beta1 = 60*pi/180;
H0 = 500;
v_x = 80;
Y0 = H0 * tan(beta1);
...
% 数据格式：[方位, 地距, 高度, n_方位, n_地距, n_高度]。
if size(target,2) ~= 6
    error(['散射点数据应为 6 列。请先运行 ship_trans.m，' ...
        '重新生成 airplane_scatter_points.mat。']);
end
target_azimuth = target(:,1) + X0;
target_ground_range = target(:,2) + Y0;
target_height = target(:,3);
target_normal = target(:,4:6);
Ntar = size(target, 1);

fprintf('使用散射点数 = %d/%d\n', Ntar, size(target_all,1));
fprintf('飞机方位向尺寸约 %.3f m，地距向尺寸约 %.3f m，高度约 %.3f m\n', ...
    max(target(:,1))-min(target(:,1)), max(target(:,2))-min(target(:,2)), ...
    max(target_height)-min(target_height));

% 简化面元响应：迎向雷达的面更亮，背向面保留少量绕射/边缘回波。
normal_power = 4;
backside_floor = 0.03;
...
% 等效模型中，真实目标通道与假目标调制通道的幅度权重。
% 保留真实通道是为了研究“真目标 + 假目标”，而不是隐身。
true_echo_weight = 1.0;
false_echo_weight = 0.8;

fprintf('设定假目标中心偏移：地距 %+.2f m，方位 %+.2f m\n', ...
    false_range_offset_m, false_azimuth_offset_m);
fprintf('真实/假目标等效幅度权重：%.2f / %.2f\n', ...
    true_echo_weight, false_echo_weight);
...
for i = 1:Ntar
    for k = 1:Na
        % 三维前向模型：雷达在 (x_radar, 0, H0)，
        % 目标点在 (方位, 绝对地距, 高度)。最终仍聚焦成二维 SAR 图像。
        look_vector = [x_radar(k)-target_azimuth(i), ...
            -target_ground_range(i), H0-target_height(i)];
        r_inst = norm(look_vector);
        look_unit = look_vector / r_inst;

        cos_incidence = max(0, dot(target_normal(i,:), look_unit));
        facet_gain = backside_floor + ...
            (1-backside_floor)*cos_incidence^normal_power;
        scatter_amplitude = facet_gain * (R0/r_inst)^2;
...
% 直接根据相位指令选择 0 或 pi。由于序列为实数方波，
% 会同时产生正、负调制阶，因此通常出现一对对称假目标。
modulation_1bit = ones(size(phase_command));
modulation_1bit(cos(phase_command) < 0) = -1;

sr_false_1bit = sr_plain .* modulation_1bit;
sr_metasurface = true_echo_weight*sr_plain + ...
    false_echo_weight*sr_false_1bit;
