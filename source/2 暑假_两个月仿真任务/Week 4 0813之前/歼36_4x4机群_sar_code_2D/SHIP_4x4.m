...
%% 1. 运行配置
% 4×4 战斗机群：四角为真实歼-36，其余 12 个格点由二维 1-bit
% 时空编码的谐波产生。距离向和方位向分别使用独立的 0/pi 编码。
resolution_mode = 'uav';
display_dynamic_range_db = 30;
% 主展示图保留到未调制单机峰值以下25 dB。20 dB适合判断目标
% 是否存在；25 dB能额外保留弱目标内部约5 dB的散射结构。
display_keep_threshold_db = 25;
display_thresholds_db = [15, 20, 25, 30];
...
% 每一个维度都采用 0 状态 30%、1 状态 70% 的非平衡 1-bit 编码。
% 非平衡编码具有非零直流项，因此四角真实目标仍会保留。
% 这里不额外假设固定反射效率，只研究1-bit编码本身的能量分配。
...
output_root = fullfile(script_dir, 'results_4x4');
...
%% 2. 雷达、场景和分辨率参数
fprintf('正在生成 4×4 歼-36机群的 SAR 原始回波...\n');
...
% 所有图统一以未调制单架飞机的成像峰值为0 dB。这样不同门限之间
% 可以直接比较，也能与理论谐波幅度表保持同一参考。
...
title(sprintf('四角真实歼-36（间距 %.1f m，未调制单机峰值=0 dB）', ...
    grid_spacing_m));

figure_deception = createResultFigure( ...
    'Figure 3 - 二维 1-bit 编码形成4×4机群（推荐门限）');
plotSarDb(img_deception_1bit, Range_axis_ground, Az_axis, ...
    display_keep_threshold_db, single_aircraft_reference, ...
    display_half_width_m);
title(sprintf(['二维 1-bit 电磁欺骗：0状态 %.0f%% / 1状态 %.0f%%，' ...
    '显示下限 -%g dB'], ...
    100*zero_state_fraction, 100*(1-zero_state_fraction), ...
    display_keep_threshold_db));

figure_deception_full = createResultFigure( ...
    'Figure 4 - 二维 1-bit 编码完整视场（含杂散）');
plotSarDb(img_deception_1bit, Range_axis_ground, Az_axis, ...
    display_dynamic_range_db, single_aircraft_reference, ...
    display_full_half_width_m);
title('二维 1-bit 电磁欺骗完整视场：正负镜像与高阶谐波杂散');

figure_thresholds = createResultFigure( ...
    'Figure 5 - 4×4机群不同dB保留门限对比');
tiledlayout(2,2, 'TileSpacing','compact', 'Padding','compact');
for threshold_index = 1:numel(display_thresholds_db)
    nexttile;
    current_threshold_db = display_thresholds_db(threshold_index);
    plotSarDb(img_deception_1bit, Range_axis_ground, Az_axis, ...
        current_threshold_db, single_aircraft_reference, ...
        display_half_width_m);
    title(sprintf('显示下限 -%g dB', current_threshold_db));
end
sgtitle('4×4机群门限对比（未调制单架飞机峰值=0 dB）');

%% 9. 30%/70% 1-bit 编码各次谐波幅度
...
thresholds_png = fullfile(run_dir, ...
    'sar_4x4_threshold_comparison.png');
thresholds_fig = fullfile(run_dir, ...
...
if exist('exportgraphics', 'file') == 2
    ...
    exportgraphics(figure_thresholds, thresholds_png, 'Resolution', 200);
    ...
    saveas(figure_thresholds, thresholds_png);
...
savefig(figure_thresholds, thresholds_fig);
...
save(mat_file, 'img_single_reference', 'img_real_corners', ...
    'img_deception_1bit', 'single_aircraft_reference', ...
    ...
    'display_keep_threshold_db', 'display_thresholds_db', ...
